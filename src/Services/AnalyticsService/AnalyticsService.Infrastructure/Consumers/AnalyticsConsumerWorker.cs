...
public sealed class RabbitMqOptions
{
    ...
    public string QueueName { get; set; } = "analytics_service_queue";
}
...
public class AnalyticsConsumerWorker : BackgroundService
{
    ...
    public override Task StartAsync(CancellationToken cancellationToken)
    {
        if (!_options.Enabled)
        {
            _logger.LogInformation("RabbitMQ analytics consumer is disabled in settings.");
        ...
        try
        {
            ...
            _channel.QueueBind(_options.QueueName, _options.ExchangeName, nameof(JobPostedEvent));
    ...
    protected override Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        consumer.Received += async (model, ea) =>
        {
            try
            {
                ...
                _logger.LogInformation("Analytics received event {EventType}", type);

                using var scope = _serviceProvider.CreateScope();
                var service = scope.ServiceProvider.GetRequiredService<AnalyticsAppService>();

                if (type == nameof(JobPostedEvent))
                {
                    await service.HandleJobPostedAsync(stoppingToken);
                }
                else if (type == nameof(ApplicationSubmittedEvent))
                {
                    ...
                    if (@event != null)
                        await service.HandleApplicationSubmittedAsync(@event.JobId, @event.RecruiterId, stoppingToken);
                }
                else if (type == nameof(ApplicationStatusChangedEvent))
                {
                    ...
                    if (@event != null)
                        await service.HandleStatusChangedAsync(@event.JobId, @event.RecruiterId, @event.NewStatus, stoppingToken);
                }
                else if (type == nameof(InterviewScheduledEvent))
                {
                    await service.HandleInterviewScheduledAsync(stoppingToken);
                }
            ...
            {
                _logger.LogError(ex, "Error processing event in Analytics");
